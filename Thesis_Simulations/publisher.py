...
# the peer review board selects reports for publication and returns the updated scientific record
def peer_review_with_scale(participants, scientific_record, rel_pl_data_val, rel_pl_surprise_val, rel_pl_bias_val, num_draws, scale):  
    actor_optimality = 1
    id_to_prob = {}
    total = 0
...
#         print(probabilities)
        ...
        for p in participants:
            if p.id == selected_participant:
                # CRUCIAL: THIS IS WHERE THE SCALE COMES IN!!!!!
                scientific_record[p.bin_choice][0] += p.reported_results["0"] * scale
                scientific_record[p.bin_choice][1] += p.reported_results["1"] * scale
